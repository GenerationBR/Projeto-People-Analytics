...
/* Rótulos curtos para o eixo X em telas pequenas — os originais estouram a área do SVG mesmo inclinados */
const ESCOLARIDADE_LABEL_MOBILE = {
  "Pós-grad./Mest./Dout.": "Pós-grad.+",
...
function HBars({ data, xKey, yKey, domain = [0, 40], height = 240, colorFn, refLine, suffix = "%", width = 130, barSize = 14 }) {
  ...
  // A largura da coluna de rótulos NÃO pode encolher agressivamente: o SVG corta
  // qualquer texto que ultrapasse essa área (overflow:hidden é o padrão em <svg>).
  const effWidth = isMobile ? Math.max(width - 20, Math.round(width * 0.88)) : width;
  const effBarSize = isMobile ? Math.min(barSize, 11) : barSize;
  const tickFont = isMobile ? 9.5 : 11;
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ top: refLine != null ? 26 : 8, right: isMobile ? 16 : 30, left: 4, bottom: 0 }}>
        <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" horizontal={false} />
        <XAxis type="number" domain={domain} {...axisProps} tick={{ fontSize: tickFont, fill: C.inkMuted }} tickFormatter={(v) => `${v}${suffix}`} />
        <YAxis type="category" dataKey={yKey} {...axisProps} tick={{ fontSize: tickFont, fill: C.inkMuted }} width={effWidth} />
        ...
        <Bar dataKey={xKey} radius={[0, 6, 6, 0]} barSize={effBarSize}>
          {data.map((d, i) => <Cell key={i} fill={colorFn ? colorFn(d, i) : C.dark} />)}
          <LabelList dataKey={xKey} position="right" formatter={(v) => `${v}${suffix}`} fill={C.ink} fontSize={tickFont} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
function RankingInstituicoesChart() {
  const isMobile = useIsMobile();
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={rankingInstituicoes} layout="vertical" margin={{ top: 8, right: isMobile ? 16 : 28, left: 4, bottom: 0 }}>
        <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" horizontal={false} />
        <XAxis type="number" domain={[0, 22]} {...axisProps} tick={{ fontSize: isMobile ? 9.5 : 11, fill: C.inkMuted }} tickFormatter={(v) => `${v}%`} />
        <YAxis type="category" dataKey="nome" {...axisProps} tick={{ fontSize: isMobile ? 9.5 : 11, fill: C.inkMuted }} width={isMobile ? 52 : 70} />
        <Tooltip {...tooltipStyle} formatter={(v) => `${v}%`} />
        <Bar dataKey="pct" radius={[0, 6, 6, 0]} barSize={isMobile ? 12 : 16}>
          {rankingInstituicoes.map((_, i) => <Cell key={i} fill={i === 0 ? C.deepest : C.dark} />)}
          <LabelList dataKey="pct" position="right" formatter={(v) => `${v}%`} fill={C.ink} fontSize={isMobile ? 9.5 : 11} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
...
/* Ticks longos (faixa salarial, escolaridade, experiência, modelo de trabalho) inclinam no mobile
   para não sobrepor — o SVG corta texto que ultrapassa a área do eixo. */
function mobileTickProps(isMobile, fontSize) {
  return isMobile
    ? { angle: -45, textAnchor: "end", height: 58, tick: { fontSize: Math.min(fontSize, 8.5), fill: C.inkMuted } }
    : { tick: { fontSize, fill: C.inkMuted } };
}
  ...
  return (
    <div className="page">
      ...
      <div className="chart-grid cols-2">
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={modeloTrabalhoDados} margin={{ top: 8, right: 8, left: isMobile ? 20 : -8, bottom: 0 }}>
              <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="modalidade" {...axisProps} {...mobileTickProps(isMobile, 10.5)} interval={0} />
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={experienciaDados} margin={{ top: 8, right: 8, left: isMobile ? 20 : -8, bottom: 0 }}>
              <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="faixa" {...axisProps} {...mobileTickProps(isMobile, 10)} interval={0} />
      ...
      <div className="chart-grid cols-2">
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={faixaSalarialDados} margin={{ top: 8, right: 8, left: isMobile ? 20 : -8, bottom: 0 }}>
              <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="faixa" {...axisProps} {...mobileTickProps(isMobile, 9.5)} interval={0} />
              <YAxis {...axisProps} tickFormatter={(v) => `${v}%`} />
              <Tooltip {...tooltipStyle} formatter={(v) => `${v}%`} />
      ...
      <div className="chart-grid cols-2">
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={escolaridadeLiderancaTIC} margin={{ top: 8, right: 8, left: isMobile ? 20 : -16, bottom: 0 }}>
              <CartesianGrid stroke={C.pale2} strokeDasharray="3 3" vertical={false} />
              <XAxis
                dataKey="nivel" {...axisProps} {...mobileTickProps(isMobile, 9.5)} interval={0}
                tickFormatter={isMobile ? (v) => ESCOLARIDADE_LABEL_MOBILE[v] ?? v : undefined}
              />
              <YAxis {...axisProps} tickFormatter={(v) => `${v}%`} />
              <Tooltip {...tooltipStyle} formatter={(v) => `${v}%`} />
  ...
  return (
    <div className={`dei-dashboard${darkMode ? " dark-mode" : ""}`}>
      <style>{`
        ...
        .dei-dashboard { overflow-x: hidden; max-width:100vw; }
        .topbar {
          display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding: 14px 36px; background: var(--white);
          border-bottom: 1px solid var(--pale2);
          position: sticky; top:0; z-index:20;
        }
        .brand { display:flex; align-items:center; gap:10px; min-width:0; }
        .brand-logo { height:42px; width:auto; object-fit:contain; border-radius:6px; display:block; flex-shrink:0; }
        .brand-text { display:flex; flex-direction:column; line-height:1.1; min-width:0; }
        .brand-title { font-family:'Fraunces', Georgia, serif; font-weight:600; font-size:16px; color:var(--deepest); letter-spacing:.01em; white-space:nowrap; }
        .brand-sub { font-size:10.5px; color:var(--inkMuted); text-transform:uppercase; letter-spacing:.08em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        ...
        .hamburger-btn {
          ...
          color:var(--deepest); cursor:pointer; transition:all .15s ease;
        }
        .hamburger-btn:hover { background: var(--pale); }
        ...
        @media (max-width: 880px) {
          .topbar { padding: 12px 20px; position:relative; }
          .nav { display:none; }
          .nav.nav-open {
            display:flex; flex-direction:column; gap:2px;
            position:absolute; top:100%; left:0; right:0;
            background:var(--white); border-bottom:1px solid var(--pale2);
            padding:10px 14px 16px; box-shadow:0 12px 24px rgba(43,22,32,.14);
            max-height:calc(100vh - 64px); overflow-y:auto;
          }
          .nav.nav-open .nav-btn { width:100%; justify-content:flex-start; padding:11px 14px; font-size:13.5px; border-radius:12px; }
          .hamburger-btn { display:flex; }
          .nav-scrim { display:block; position:fixed; inset:0; background:rgba(43,22,32,.28); z-index:9; }
        ...
        @media (max-width: 560px) {
          ...
          .brand-sub { font-size:9px; max-width:150px; }
          .dark-toggle, .hamburger-btn { width:32px; height:32px; }

          .page { padding: 18px 14px 40px; }
          .page-title { font-size:20px; }
          .page-sub { font-size:11px; }

          .kpi-grid, .kpi-grid-3, .kpi-grid-2 { grid-template-columns:1fr; gap:10px; }
          .kpi-card { padding:14px 16px; }
          .kpi-value { font-size:22px; }

          .chart-grid { gap:10px; margin-bottom:10px; }
          .chart-card { padding:14px 14px; margin-bottom:10px; }
          .chart-card-title { font-size:13px; }
          .chart-card-sub { font-size:10.5px; }

          .donut-legend { grid-template-columns:1fr; }
          .funnel-row { padding:11px 14px; width:100% !important; }
          .funnel-value { font-size:16px; }
          .rung-label { width:96px; font-size:10.5px; }
          .rung-value { width:24px; }

          .section-divider { margin:22px 0 12px; padding-top:16px; }
          .section-divider-label { font-size:14px; }
      ...
      <div className="topbar">
        ...
        <div className="topbar-actions">
          ...
          >
            {navOpen ? <X size={19} strokeWidth={2} /> : <Menu size={19} strokeWidth={2} />}
          </button>
        </div>
      </div>
